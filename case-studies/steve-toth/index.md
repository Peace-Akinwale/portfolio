...
---
## 2026-08-25, twenty-nine lying success messages, a repair agent, and an audit that caught my own regression

The client's operator asked one question after the previous night's incident: "what else is lying to me in my system?" This day answered it with evidence across three live products, made the lie class structurally impossible where it mattered most, built her design for a self-repairing pipeline, and closed with a fresh-eyes audit of my own work that found a critical regression I had shipped without knowing.

### What shipped

- A three-product truth audit: every user-facing success claim traced back to what was verified before it fired. 27 claims could fire false (13, 5 and 9 per product) plus 3 wrong-failure messages. All fixed the same day, each product verified with its full test suite (1,253 / 1,638 / 1,241 tests green at close) and live checks.
- An outcome-report architecture in the main product: every pipeline step returns ok or a failure with its reason and retryability, one composer renders every notification from that report, a missing report renders as unverified rather than success, and reports persist to the database so a later page load tells the same truth as the original push. Combination tests enumerate wording against outcome so no phrasing can claim an unverified step.
- Five-tier matching for pasted social links, built live against three consecutive production failures of the same post in one evening: title key, then text key, then a 3-day window from the link id's embedded timestamp, then a 36-hour window after measuring that two of the platform's id namespaces mint within 300 milliseconds of publish, then id-to-id mint proximity. The client's four-times-refused post landed on the third deploy; the pasted and stored ids decode 375 milliseconds apart.
- The operator's own design, built the night she described it: a Tier-1 repair agent. A failing add is examined by a model with the same evidence a human engineer used, fenced against instructions embedded in post text, sanity-checked in code, completed through the normal verified path with a notification that says a repair happened and why, or refused honestly with the operator mentioned by name in Slack.
- A three-week edit-pattern analysis of the client's newsletter, delivered as a branded interactive page: 26 concrete edits he made to generated drafts, distilled to 12 prompt-encodable patterns, each validated against three published weeks before he was asked to review anything.

### Decisions worth recording

- **A missing report reads as unverified, never as success.** The composer's default for absent evidence is the failure copy. A future engineer who forgets to thread the report through degrades the system to honesty, not back to the original lie.
- **Refusal beats guessing at every tier, including the model's.** The repair agent's confident pick is overruled by a code-side timestamp envelope; twins the evidence cannot split stay refused. The failure mode of a wrong guess is publishing the wrong author to a client-facing roundup, which is strictly worse than an honest no.
- **The operator's failure channel was scoped down on audit evidence.** Her pings ride a webhook into a channel the client watches, so they fire only on repair-path failures, not on routine not-founds. Reach was traded for signal deliberately, and documented.
- **Agent-written code fixes were designed and deliberately not built.** The operator wants eyes on anything that ships to production; Tier 2 waits for her call.
- **Cross-week validation before the client sees a pattern list.** Two of twelve patterns from the single-week read were wrong as first written; the published weeks corrected them before they could steer his prompt.

### Frictions and course corrections

- **The audit of my own work found that I had silently reverted the client's own requested fix.** Ten days earlier, a session had hardened a drafting spec by editing a generated file instead of its source. My unrelated regeneration wiped it, tests stayed green, and production quietly returned to the behavior the client had explicitly complained about. Only a fresh-eyes diff audit caught it. The fix went into the source file so no regeneration can lose it again, and the lesson became a written detector pattern in the reusable bug-hunting library.
- **My own remediation contained one of the bugs it was fixing.** The banner-verification change I wrote destructured a database result without reading its error field, the exact violation the day's rules were built against. The audit caught my fix's own defect.
- **Two design iterations missed the operator's point before a voice memo landed it.** My first self-healing proposal handled incidents without fixing anything; my second learned rules without investigating. Her framing, an engineer who diagnoses, completes the user's original ask, and narrates what happened, became the architecture nearly verbatim. Restating her idea back until she confirmed it was the step that worked.
- **A build broke in CI for a reason no test could see.** Moving a helper out of a server-actions file is a framework build rule invisible to the type checker and test suite. Fixed within minutes; the next push verified the production build locally first.
- **Three deploys in one evening for one bug.** Each matching tier was tested live against the real failing row before designing the next, and the two failed intermediate attempts cost about two cents of scraper spend. Working the live case beat speculating about it.
...
- Eight root fixes, each with a test, in four commits on `stevetoth/fanout-notebook` (`d1d6fa5`, `e2296a2`, `5ddf172`, `8e8f6f1`): 204 to 225 tests, parity 6 of 6, CI run 33183732219 green on the third. A fifth commit the same evening added the parser half of a new unit (per-claim winners and runner-ups, search passes with none of the client's pages, brands the engine named in its own queries) with one additive migration applied to the live schema: 241 tests.
...
- **The second real fixture arrived that evening and broke the adapter twice.** The operator saved the Perplexity JSON by hand (94 KB). The adapter had been reading sources from a key the real payload does not use, so it saw 7 queries and 0 pages and the classifier still called the result "parsed" because queries existed; and Perplexity numbers its citations by the final answer's own source list, which differed from the adapter's assumed order at every position, so every citation would have been attributed to the wrong page. Both fixed at the root (the classifier now refuses "parsed" when result steps yield no page, on both stored-route engines), pinned against the real numbers: 7 queries, pool 26, 8 markers all resolved, 8 cited. All three engines now parse real payloads; 225 tests.
- **A brief's premise was false and a shape test caught it before the first sync.** Late in the day the client's operator brought a competitor researcher's studies (per-claim runner-ups, brands named in the engine's own first query). The brief for that work said the new parser field needed no database change because "the parse column is jsonb". The pool table is one column per field. A test that compares the row shape against the migration files failed on the new field, the additive migration was written and applied to the live schema that evening, and the same test now folds every later migration into its check. Without it the first real sync would have failed on a missing column with the operator watching.
...
- Claude and Perplexity measured live: Claude's stop control and streaming flag are what the guesses hoped; Perplexity shows no stop control at all, so the turn count carries the trigger, and the first-turn rule was fixed the same evening. Later that night: Sources became one flat list on her ask, and the dashboard gained a web app manifest, icons and a static-only service worker so teammates can install it on a phone once deployed (proven locally; data is never cached). 314 tests, CI green.
...
- **The file the operator remembered sending was the wrong one.** Asked to find "the audio I sent Karla", the first search surfaced a June file from the DM; the operator said no. Tracing her own 7 August nudge back to its linked message found the real one (3 August, a different file entirely, the one that had produced the better clone). The June file would have reproduced the rejected round-one clone. The task now carries the right audio, embedded, with the two facts side by side.
...
## 2026-09-04, a product built in a day, and three bugs that would have shipped it broken to a paying client

Steve asked in Slack for "the cheapest and simplest way to give their own version of the notebook okf with their data", pointing at a free competitor tool. The answer built that day was not a copy of it. The competitor crawls a website and zips one markdown file per page with no model reading anything; this reads every document a client hands over, distills it, and refuses to ship a note that merely restates its source.

### What shipped

- **The Client OKF Kit, live at `okf.notebookers.com/admin/kits`.** A strategist drops a client's folder or zip on one screen and presses Build. The output is a zip the client hosts themselves: `okf/` (their knowledge as markdown notes), `site/` (a static viewer with Browse, Map, Notes and an Ask your AI page, no build step and no server), `ask/` (a copy-paste prompt plus a portable skill for coding agents), and a README with hosting steps per platform. 13 commits, 84 new files in the first wave.
- **Nothing in the kit reaches back to us.** An export-safety gate refuses the zip if any file entering it contains one of our hosts or credentials. Verified on the real export: **zero** hits for `railway.app`, `notebookers.com`, `sk-ant` or `ANTHROPIC` across 553 entries, checked inside the nested skill zip too. So the recurring cost per client after hand-over is **$0**.
- **Measured, not estimated:** 101 documents read, 331 notes written, 877,966 input and 131,338 output tokens, **$4.60 in 48 minutes**, zero files held. That is $0.046 per document, **$4.56 per 100**, one time. The estimate that circulated before the build was $3 per 100 in 5 to 10 minutes, so it was low by about half on cost and by five times on duration. The UI and the runbook now state the measured figures.
- **Proven as a client would use it**, not just as a green suite: the exported `site/` served from a plain `python3 -m http.server`, all four screens driven in Chromium at 1440px and 390px in light and dark with zero console errors, and a real question about client onboarding answered from the hosted notes with file-path citations to eight specific files.
- **A brand system extracted to `DESIGN.md`** from Notebook Agency's guidelines, then applied to the admin screens and the client viewer: six colors and no others, Cream ground, Ink text, 8px grid, 1px hairlines, no shadows. Verified by reading computed styles rather than by eye.
- **Google sign-in on the admin login**, ported from Radar on Google Identity Services, with the source-scan tests that forbid a redirect leg coming back.
- **Verified at close:** 1,848 tests passing across 126 files (1,241 at the start), `tsc --noEmit` clean, `npm run build` clean, and `git diff` on Steve's own knowledge bundle empty across every commit.

### Decisions worth recording

- **A kit is a folder, not a tenant.** The obvious build was multi-tenant hosting: one login per client on our infrastructure. Rejected because it creates a per-client server, row and bill that Notebook pays forever. Clients host the output themselves on their own domain, which also means the client needs no account on our side, so there was nothing to authenticate and no new database object to create. The whole product adds zero tables.
- **The static viewer vendors its dependencies instead of using a CDN.** A CDN URL would break the two cases the brief specifically asks for, opening the kit from disk and hosting it privately, and it would put a third-party request on a client's own domain. No web font either: the licence for our typeface is unconfirmed, and a client's site should not load our fonts to render their own knowledge.
- **A validator we ship to the client has to pass on the artifact we hand them.** The portable skill inside every kit includes a bundle checker, and it exited 1 on every kit, reporting the indexes "stale" because its own generator would have written them differently. The kit was conformant; the bundled tool was judging our output by its own formatting. Made that sub-check advisory and kept the real gates hard, because a tool that tells a paying client their knowledge base is broken is worse than no tool.
- **Red is reserved for the one state that stops a human.** Asked to apply a six-color brand palette to screens that report build states, the temptation is a green "done" and an amber "in progress". Both are outside the palette, and the guidelines say so twice. Every chip now carries its own word in Ink and Mist, and Red belongs only to "Held", the state a strategist has to act on. The chips work in grayscale as a result.
- **One palette conflict was flagged rather than silently resolved.** The guidelines say to reach for the light neutral instead of a tinted dark when a softer value is needed. That is right for borders and wrong for text: measured, the neutral as text on the Cream ground is 1.34:1, which fails accessibility outright, while a muted ink measures 6.06:1. The client viewer took the strict path and has zero off-palette values; the admin app keeps two muted text tokens, documented with the measured contrast and the note that collapsing them is a two-line change if the brand owner wants it.
- **Phase 2 was described and deliberately not built.** Running the whole app on a client's own hosting needs a no-database mode, cookie sessions, and their own OAuth client. Half of that shipped is worse than none, so it is specified in the handoff instead.

### Frictions and course corrections

- **The first kit built could never have been exported, and three separate bugs caused it.** A payment-card guard held two ordinary marketing blog posts, because a 19-digit Twitter post id passes the card checksum about one time in ten and the guard checked nothing else. A held file blocked the export, and nothing in the interface could clear the hold, though the screen told the strategist to remove the file. And the export gate banned the *words* "Anthropic" and "Supabase" inside client-authored notes, which for an AI and SEO agency's clients is everyday content, while our own hold message carried the word "Anthropic" into the exported log and made the kit permanently unexportable by our own vocabulary. All three root-fixed; the second build held zero files.
- **A client's knowledge base opened by naming Steve.** The generated index read "Root index of Steve Toth's Notebook OKF v0.2 knowledge bundle" and headed its sections with internal type names. That file is the first thing a client's team and their AI read. The generator now takes the client's own name and the reader vocabulary, with the old strings kept as defaults so Steve's own bundle is byte-identical, which its existing tests proved.
- **That fix was committed and the zip shipped the old wording anyway, twice.** A 48-minute build was holding the previous version of the module in memory, and the exporter regenerated the index only when an unrelated cleanup had removed something. Root fix: regenerate deterministic artifacts at the boundary that ships them, unconditionally, so the artifact is a function of the current code rather than of whichever process happened to write it. Written up as a reusable detector, because "the fix is committed" and "the customer receives the fix" are independent facts.
- **The phone map was unreadable and passed every automated check.** 150 labeled nodes on a 390px screen overlap into a mat that reads as a broken render. Zero console errors, correct colors, correct contrast, and useless. A phone now gets 40 unlabeled nodes with the count stated honestly, because a tap already shows the full title, so the labels were a second way to say the same thing and the first way to ruin the picture.
- **The end-of-day report said Google sign-in was live and working. It was not.** Eight source-scan tests were green, the button rendered, the client id was in the bundle, and the sign-in failed inside Google's popup because the app's JavaScript origin had never been registered in the console. That is a human step no test, build or deploy can see, and the brief had named it in advance. Corrected in the handoff rather than quietly fixed.
- **Reading the auth layer afterwards found a bug nobody had reported.** The Google button navigated on any successful token exchange without checking whether the account was approved, so an existing but unapproved teammate would get a valid session, land on a blank login page, and have no idea why, with the cookie still live. It is the exact failure the password path had already fixed for itself, whose code comment describes the symptom; the Google button was written later and did not copy it. Security was never affected, the person was simply never told. Fixed with a server route that answers only about the caller and fails closed.
- **A cost figure in the documentation was a blend of two different builds.** The first build's document count and duration had been quoted alongside the second build's spend and notes, and three files carried three different per-100 numbers. Caught by re-running every number before writing this entry rather than trusting the earlier write-up. These figures get quoted to a client, so a blend is worse than a round number.
- **A note in the shared memory said an API token was dead, and it was not.** The conclusion had been drawn from the one endpoint that file itself warns not to judge the token by. Re-probed: the token authenticates fine and simply has no DNS permission at all, which is the real reason a record had to be added by hand. The distinguishing test is now written down, and no replacement token is needed.

### Why this matters for the portfolio

- The gap between "the tests pass" and "the customer receives what we intended" is where this day's real work happened. Three of the four worst defects were invisible to a green suite: a bundled validator that only fails on the customer's machine, a stale artifact written by a process that predated the fix, and a map that renders perfectly and cannot be read.
- Building the thing was one day; the audits were the value. A pre-emptive bug hunt and a completeness audit run before the demo found the two bugs that made the first kit unexportable and the one that put our founder's name in a client's file. None had been reported by a user, because no user had received it yet.
- A cost number is a product decision. Measuring $4.56 per 100 documents rather than repeating an estimated $3 changes what the agency can quote and whether the service is profitable, and the honest number was almost double.
- Refusing scope is part of delivering it. Multi-tenant hosting, a CDN dependency, a web font, and phase 2 were each the easier build and each rejected for a stated reason. What shipped costs the agency nothing per client and puts the client's own name on their own knowledge.

---

## 2026-09-05, an outage caused by two workers asking an empty queue 104,000 times a day, and the week the wrong metric got fixed

Steve forwarded a Supabase email at 22:40 the night before: service restrictions active on the organization. Four products lost sign-in at once, including a portal his paying coaching students use. The cause had been running since July, and a fix shipped four days earlier had addressed a different problem entirely.

### What shipped

- **The root cause, fixed and measured live.** Two queue workers polled every 5 seconds, and each idle tick issued three database requests rather than one, because a cleanup pass meant for jobs stuck longer than 10 minutes ran on every single check. That is roughly 104,000 requests a day against an empty queue, 63% of all traffic to the database. Recovery moved onto its own 5-minute clock. Measured on the live system with the same minute-bucketed query before and after: **72 requests and 48 writes per minute became 24 requests and 0 writes**, with a 4-write recovery pulse every fifth minute, held flat across 54 consecutive minutes and through a redeploy.
- **A second leak, closed.** A notification badge re-checked every 60 seconds per open browser tab, and each check fanned out to about 8 database lookups, so a single forgotten tab cost around 12,000 requests a day. Hidden tabs now poll nothing and refresh the moment they become visible; the route also keeps a 30-second shared cache so many tabs collapse to one read.
- **A usage alarm, because there is no bill to read.** Supabase's public Management API does not expose usage at all: both organization endpoints return 404, tested rather than assumed, and the dashboard's 7, 14 and 28 day reports are paid-plan only. The alarm reads `pg_stat_statements` instead, snapshots request counts daily into a new table (migration 075), projects to the end of the billing cycle, alerts only on a band change at 60% and 85%, and names the fastest-growing query so the message says what is burning the quota rather than only that something is.
- **Verified at close:** 1,353 tests passing across 128 files, `tsc --noEmit` clean, migration 075 applied to production, the RPC returning real data (7,709,108 calls across 39 tracked statements), the crawler deployment reporting SUCCESS with the alarm registered in its boot log. 6 commits.
- **Documentation routed by layer rather than piled into context:** a 102-line runbook, a 420-line handoff, two dated decision entries, and a net **+676 bytes** to the always-on project file despite adding a whole new operational rule.

### Decisions worth recording

- **Fix the request count per tick, not the poll interval.** The obvious fix was slowing the 5-second poll. Rejected: that 5 seconds is why a pasted link appears almost immediately, which is the one moment the client actually uses the feature. Moving the cleanup off the hot tick removed two thirds of the traffic and changed nothing a user can perceive.
- **The alarm projects rather than reports.** "You are at 61% on the 20th" is not actionable. "At today's rate you will finish the cycle at 140%" is. It also fires only when it crosses a band, never on every run, because a daily "still at 61%" is exactly how an alarm teaches people to ignore it.
- **Tests were changed to match the measurement, not the other way round.** A regression test asserted the outage-day traffic would read as critical. It came back as warn, at 75%. The threshold stayed and the test changed, because bending a threshold to make a test pass would have buried the real number. That 75% is itself the finding: the two idle workers alone would have tripped a warning weeks before anything broke.
- **The upgrade was not purchased.** Restoring service needed a plan change on the client's card. That is his decision and his money, so the work was to root-cause it, fix it, quantify it, and put the ask in front of him with the exit routes attached.
- **Documentation was corrected in place rather than appended.** The project's always-on file still carried the claim that the quota threat was closed. Left alone it would have told the next session the problem was handled. It was superseded inline with the reason, and the space for the new content was paid for by relocating a narrative that already existed in three other files.

### Frictions and course corrections

- **The previous week's fix solved the wrong problem, and this was mine.** An earlier warning was read as a storage problem and answered by migrating 700 MB of media to Cloudflare. That work was correct and it held. But the metric actually failing was bandwidth, and it was never re-checked. On the quiet day before the outage the split was **database 140.692 MB at 97.1%, storage 435.851 KB at 0.3%**. The lesson is narrow and reusable: "over quota" names a metric, and the dashboard has to be read before anything is touched.
- **One organization's quota covered two databases and four products.** The coaching portal's own bandwidth was measured in kilobytes. It went down anyway. Any reasoning of the form "this only affects one app" was wrong by construction, and nothing in the codebase said so until now.
- **The most promising suspect was wrong.** A query returning every column of a large table looked like the answer: 83.1% of each row was a scrape payload the interface never displays, 14,262 bytes of dead weight per row. It was ruled out by dating the commit that had already fixed it, three weeks before the spike. Confident and wrong would have closed the investigation early.
- **An audit of the alarm found a real bug in the alarm.** It divided the cycle's traffic by days elapsed rather than days actually measured. Going live on day 23 of a cycle with no history, its first reading would have spread one day of traffic across 23 days and reported about 3% for a rate that truly lands near 75%. A falsely low alarm is worse than none, because it looks like an answer. Found by re-reading the code when asked to audit rather than by any test.
- **A verification script reported success when it had timed out.** The background check exited zero on its fallback command, and the pass was almost reported on that basis. The deployment record was queried directly instead. Corrected out loud in the same message.
- **The first draft of the client email was accurate and got deleted.** It explained the cause, the fix and the numbers, and it never said plainly that this should have been caught. Rewritten to open on that, then the mechanism, then the fixes, then why none of it restores service immediately, then the business case with two exit routes.
- **The explanation of the badge leak failed three times.** Each attempt described the mechanism correctly and still did not land, because the number was never derived: 1,440 checks a day multiplied by about 8 lookups per check is where 12,000 comes from. Clarity was not a matter of shorter words, it was showing the arithmetic.
- **The spike that started it could not be explained, and the entry says so.** Four days carried an extra 2.2 GB. Content volume was flat across them, so it was read traffic, and the badge polling is the best available mechanism. The request-level records are gone: the free plan retains 24 hours, and the longer reports are locked behind the paid tier. It was written up as inferred rather than proven, in the client email as well as internally.

### Why this matters for the portfolio

- Root cause was established against live traffic, not by reading code and forming a theory. The prediction from the source was 103,680 requests a day; the measurement was about 104,000. Fixes were then proven the same way, before and after, on the same query.
- The most useful output was not the fix, it was the alarm. A leak ran for weeks and the first signal anyone had was the client forwarding an outage email. Building the detector was treated as the actual deliverable, and it was built on the data that exists rather than the data that should exist, once the billing API turned out to expose nothing.
- Auditing your own work has to be able to find your own bugs. The alarm shipped, was audited hours later, and the audit found a defect that would have made it silently useless. It was fixed and disclosed rather than left to be discovered by the next outage.
...
---
## 2026-09-08, a design complaint that surfaced an outage, and a client who asked for a method instead of a revert

The session opened with the client at the end of her patience over one screen's design and closed with a written rule for how any screen may change. In between, her report that she "can't delete newsletters I don't want" and "can't process the ones I accepted" turned out to be three separate production faults in the approval queue of the knowledge-base product, one of which had silently marked fifteen of her own approvals as rejected.

### What shipped

- **Six code commits and one handoff to production** in a working evening (`ee8e8f5`, `ace2e22`, `84fd774`, `f0601b4`, `2c48120`, `0643e33`, `4072295`), each auto-deployed to Railway and each verified by matching the newest successful deployment's commit hash against the branch, not by polling a static health route. Live check at close: 200.
- **Root cause one, a three-second database scan.** Every queue filter tested a field inside a JSON document (`value->>'status'`). The table held 106 rows but 22 MB, because each stored proposal is roughly 200 KB, so the filter had to decompress every row to read one short word: measured 3,245 ms on an idle connection against the platform's 8,000 ms statement ceiling. The same filter on the generated column that a migration had already created the day before: 2 ms. It was also the single largest processor consumer on a database shared with three other products, at 331 seconds of total execution time.
- **A mitigation applied before any deploy.** An expression index added by hand at 16:49 UTC stopped the timeouts that minute, then was recorded as a numbered migration so a fresh database inherits it.
- **Root cause two, an hourly budget spent by a fan-out.** Each publish re-read all 737 files of the knowledge bundle from the GitHub API, one request per file, per commit and again per retry. Three approvals exhausted the token's 5,000 requests an hour; the log shows 332 refusals inside one minute. The publish path now reads unchanged files from the copy the deploy already carries, comparing each file's git content hash, and fetches only what differs: roughly 740 requests per publish became roughly 20, confirmed in the production log as "732 from disk, 0 fetched".
- **Root cause three, found by watching the recovery rather than by reasoning.** Every publish redeploys the application by design, which destroys the process holding the next publish. A boot reconciler now settles those orphaned rows three minutes after start-up with a single request, deciding each one by whether its commit is actually on the branch, then restarts the queue.
- **Fifteen destroyed approvals recovered.** Twelve wrongly rejected rows and three quarantined ones returned to approved, one row frozen for a day returned to the queue, and three exact duplicates were deliberately left rejected and labelled as duplicates rather than published twice. All fifteen have since published: nineteen publish commits landed between 17:38 and 18:56 UTC, and the queue closed with nothing in flight.
- **A product file that did not exist before.** `PRODUCT.md`, 65 lines: one sentence saying what each of eleven screens is for, what each is judged by, and five rules for when a screen may change at all.
- **The knowledge graph restored to the palette the client missed**, with three deliberate departures recorded in the design system, and the palette is now formally her decision: the token test fails the build on any new colour.
- **The admin screen edited rather than rebuilt**: its queue panel went from 584 lines to about 250, two files were deleted, and a decision control now sits on every waiting row. Suite green at 2,502 tests across 165 files, types clean.
- **Two reusable bug patterns added to the bug-hunting skill** (C57, C58), each with the detector that would have caught this before a person did.

### Decisions worth recording

- **A rate limit is a clock, not a failure of the work.** This is the load-bearing decision. The retry policy had classified the refusal by status code alone, so it retried within seconds against an hourly window, counted each doomed pass against the item's three-attempt limit, and then marked work a person had explicitly approved as rejected. The smaller fix, adding that status to the terminal list, was rejected because it would have destroyed approvals faster instead of never. A spent budget now pauses the lane, counts no attempt, and resumes on its own.
- **Refused to revert a screen on taste, and proposed a method instead.** The client said the admin screen had been better the week before and offered to have it reverted. She then declined her own instruction: "let's not do this based on what I say. what's a good way to handle this?" The answer was that every screen in the product had been rebuilt at least twice in five days, each time transcribing a prototype or acting on an audit, with nothing measured and no image of the previous state kept, so each redesign was a coin flip on taste. Her dissatisfaction was the correct response to a screen with no stated job. Five rules were written and accepted: a screen changes only when a person cannot do its job or does it slower; edits happen in place, one at a time, so a fix and a restyle can be kept apart; a full-page image of the previous state is committed with every visual change; taste is treated as data ("what did you do there that is harder now?") rather than as a specification; and nothing stays on a screen that a person does not act on.
- **Her preference for one screen was honoured as data, not as a blanket revert.** She said the graph was the only thing she felt personal about. Its palette was restored from the pre-change file rather than reinvented, with three departures stated as measurements: the current brand red instead of a retired one, an ink selection ring because a red ring on a red node is invisible, and the page's own background so there is no seam against the application shell.
- **The client's own screen redesign, finished the same day, was held back.** It qualified for change under the first rule, since a client's own material sat 1,800 pixels down the page, but as delivered it was a rebuild and so broke the second. It is parked on a branch and inventoried as three separate edits for a later session.
- **Kept the strict refusal for genuinely unreadable content.** The new pause applies only when the provider says the budget is spent. A refusal without that signal keeps its old, stricter handling, so nothing unread can reach a client.

### Frictions and course corrections

- **Standing up last week's screen for comparison took the shared database down for a few seconds.** The old version read every stored proposal on every poll, which is the exact fault being fixed, so its logs filled with timeouts and the database host answered "web server is down". The server was killed, production was verified healthy, and the two versions were compared from source instead. That is the cost of not having kept an image of the previous state, and it is why committing one is now a rule.
- **The first admin pass broke a brand rule I had just cited.** Twenty-five red approve buttons down one page, in a system that allows one red per view. Caught on my own review before the client saw it and changed to ink.
- **A deliberate null byte made a source file binary to git.** The new layout module used a null character as a separator inside a key, so the commit reported it as binary and the diff was unreadable. Replaced with a printable character.
- **Two of the eight tests written before the fixes were wrong.** One fixture triggered an unrelated signing path; one pinned a fixed timestamp that expired the same evening the rule "never answer a time in the past" was written. Both corrected, and all eight were watched failing against the old code first.
- **A request to remove a former demo client's name was larger than it looked.** Nineteen files across library comments, test fixtures, scripts, the exported viewer, a brief, a plan, three handoffs and the decision log, plus two files outside the repository. A placeholder was chosen over deletion so the measured facts those documents cite keep their subject.
- **Screenshots would not capture at first.** A tall emulated viewport returned blank frames because the browser pane does not repaint while hidden. Switched to a scripted browser, which worked immediately, and this was already a recorded trap in my own notes.
- **A capability claim left unverified.** The tool used to probe the GitHub budget reported zero requests used while production had just spent twenty-five, which suggests the two are not the same identity. Recorded as an open question with the reliable alternative named, rather than reported as a fact either way.

### Why this matters for the portfolio

- The client reported a symptom in her own words. Reading production logs and measuring the database first, before proposing anything, separated one complaint into three unrelated mechanisms, one of which had been destroying her work silently.
- The most valuable output of the evening is not a fix, it is a rule. When a client says a screen was better before, a revert satisfies the moment and guarantees the argument recurs. She explicitly asked not to be the specification, and the answer was a written method that makes the next redesign arguable against something.
- Fifteen items a person had approved were sitting marked as rejected, blaming the item, with nothing published. That is the class of defect worth finding: the system had recorded a decision she never made.
- Every fix carries a test that was watched failing against the old code, and the two generalisable causes were folded into a reusable pattern library with their detectors, so the next project inherits them.
- Where evidence was missing, the entry says so: the token identity question is open, and a font defect on one screen is named as unexplained rather than quietly fixed.

---

## 2026-09-09, an audit of another agent's release, a demo that failed on the exact defect the audit had named, and a usage watch built without a secret

The client had spent a day and a half redesigning the team's AI-visibility dashboard with a different coding agent (Codex Desktop) and had deployed the result that morning. She asked for an audit, no code, from the other agent's own session transcripts. The audit found nine things; she added three when she said the shared database returns from a paid plan to the free tier in October. In the middle of the fix pass her live demo of the product to the whole team failed, and the request log showed the failure was the audit's top finding plus two actions of my own.

### What shipped

- **The audit** reconstructed why every change had been made from 57 of her turns in a 134 MB Codex transcript, then verified the deployed release against production: 947 unit tests and the type check run locally, health and auth probes, and every redesigned screen rendered signed in. Nine findings, ranked; the first two were that production ran from an undocumented branch six commits ahead of the documented one, and that the team's query decisions lived in a JSON file on a single Railway volume with no backup, no RLS and no SQL.
- **The fix pass, deployed the same evening** (fanout commits `fe25af0` and `4b834b3`, Railway deployment `aff062dc`; 986 unit tests across 86 files, 51 browser checks, type check and both builds green). Decisions moved into two pool tables (migration 013) that store only decisions and regenerate candidates on read; the old file imported once and the volume variable was removed. Parse bodies, 53 KB a row, were being fetched for every parser version of every capture: 116 rows for 27 captures, 6,203,648 bytes per full read. The loader now reads identities first and bodies only for the newest version, with a process cache keyed by row id plus updated_at: 1,491,367 bytes, measured in SQL before the change. The extension's two-minute metadata pull became incremental on a server watermark. Migration 014 lets Settings show which build each teammate runs, so the pending destructive migration (012) can wait on evidence instead of hope.
- **A Supabase usage report every three days**, because the platform's management API exposes no usage or billing endpoint (verified 404 on the paid plan too) and the billed egress figure exists only on a dashboard behind a login. A Radar route reads both projects through the API's SQL endpoint (sizes, storage, statement counts attributed per schema so three products sharing one database are told apart, row widths), estimates egress as a per-request header plus rows times row width, diffs against the previous run and projects the day the org would cross the free tier's 5 GB. Vercel cron computes it (migration 077); a public read-only endpoint serves the stored result; a cloud routine files it into the repo and posts a one-line verdict to her Slack. First run verified end to end: commit `354c57e`, Slack message posted, baseline on record. First real rate lands 2026-09-12.

### Decisions worth recording

- **Storage for team decisions went to the database, and the bandwidth argument for keeping it out was measured to be nothing.** Her concern was load on a database that had caused a four-app outage five days earlier. The decisions are kilobytes. The things that actually cost egress were named with numbers (the 6.2 MB parse read, the extension pull) and fixed instead.
- **Judge every read against the tier the org returns to, not the one it is on.** She said Pro is a one-month window. Every bandwidth decision in the pass is written against 5 GB a month, about 165 MB a day, with a written downgrade gate: two consecutive reports under roughly 120 MB a day and a dashboard reading that agrees.
- **The secret that could not travel was designed out.** The usage endpoint was first built behind its own bearer key. The routine that needed the key runs in a cloud sandbox with no secret store, and the session's own tooling refused to print the key into a chat transcript, correctly. Rather than weaken either, the compute moved under the cron secret the platform already presents, and the stored result became a public read-only endpoint exposing sizes and counts and nothing personal, the same pattern an existing health probe already used.
- **Kept an experimental caching flag knowingly.** The framework marks it "not recommended for production". It stays, with the two-minute worst-case staleness written into the decision log so nobody hunts a missing capture that is only cache.

### Frictions and course corrections

- **The demo.** Her Core Team meeting ran 14:00 to 14:50 UTC. Railway's request log for 14:42 to 14:44: the overview took 49.5 seconds, the query-suggestions API 52.6 seconds before the browser gave up, the library 37.9 seconds. Two minutes later the same pages answered in 50 to 140 milliseconds. The cause was the audit's finding about parse bodies: two captures landing at 14:38 and 14:39 cleared the server cache, and every next click reloaded 6.2 MB in parallel on one container. After the deploy, from her own clicks cold: overview 1.2 s, suggestions API 0.8 s, library 0.4 s.
- **The side panel "did not work" by design.** Both demo captures reached the shared pool with full answers. ChatGPT had answered the two definitional questions without searching, so there was no fan-out to show. A demo question has to force a search; that is now written down.
- **Two of the three causes were mine, and are recorded as such.** I rebuilt the extension output folder twice during her meeting; her unpacked install runs from that folder, so the rebuild reloaded her live extension and orphaned her open engine tabs. I also opened a new window in her work browser while she was screen-sharing. Both became standing rules the same hour: read her calendar and ask before any browser automation or extension build in working hours.
- **The platform's permission classifier blocked two things it should have.** Printing the bearer key, and applying a migration through a raw HTTP write. The migration went through the sanctioned database tool instead; the key was designed away rather than smuggled.
- **A rebuilt test fake was hiding a real change.** The data loader's fake database matched every IN filter against one column regardless of the column asked for, so the new by-id fetch returned nothing under test while working in production. The fake now honours the column.
- **One zero-traffic edge case produced an invalid date.** A project with no calls between two reports divided the ceiling by zero and threw inside the projection. Caught by its test, fixed to "would not cross at this rate".

### Why this matters for the portfolio

- Auditing another agent's work meant reading what the client had actually asked for, turn by turn, and separating "deployed without her expecting it" from "deployed at her word". Both happened that day, and the record says which was which.
- The demo failed on the defect the audit had ranked first that morning. The number that made the case (6.2 MB per click) was measured in SQL before the fix and confirmed in the request log after, from the client's own clicks.
- When my own actions contributed to the failure, the entry and the client both got the timestamps, not a softer story. Owning two of three causes in the same breath as fixing the third is how the trust survives the evening.
...
---
## 2026-09-12 to 2026-09-13, a speed ask that turned into trustworthy stored evidence, a roster that reads 28 rows instead of every run, and a four-day silent sign-out found in the logs

Two days on Fanout Notebook, Steve's team product for capturing what ChatGPT, Claude and Perplexity read before they answer. The first day shipped the work the previous handoff left in flight (a discovered-competitors pass, a client page with one red line, a read-only client role) and then a bug hunt and an archaeology pass over three months of history. Peace then asked for speed "just like the decision we made for Radar", for a team in Europe, North America, Asia and Nigeria. The measurement said the server was already beside the database; the structural problem was a home screen that read every run of every client to draw 28 rows. Fixing that properly took a staleness key the database maintains, one healer, a shared measures package and a stored rollup per client. On the second day Peace reported that her own captures were not reaching the dashboard, and the logs showed they had not reached it for four days.

### What shipped

- **Ten migrations, 020 to 029, applied and read back on the shared Supabase project**, every one covered by a test that runs the whole migration history against a real PostgreSQL 18 (PGlite) before it is applied: the discovered-competitors pass table, the client role, narrowed grants (nobody holds TRUNCATE, which row security cannot gate; anon holds nothing), the evidence key and its guard, the client rollups, and the view the brand pass reads.
- **Stored evidence carries the key it was derived under** (`parses.evidence_key` = client id plus a trigger-kept hash of the client's domains, aliases and competitors). Only the runner's service role may assert a key, and any other write that changes an evidence input nulls it. The runner heals stale rows on its five-minute sweep with the same `reconcileEvidence` the dashboard uses at read time, moved into `@fanout/parser` so there is one implementation. Backlog of 230 rows healed in three sweeps.
- **The Clients home reads one stored row per client.** `@fanout/measures` (12 files moved out of the dashboard with one-line shims left at the old paths, so none of 37 importers changed) lets the runner compute the roster rows with the code the dashboard renders; `fanout.client_rollups` stores them under an input key the database computes; a `security_invoker` view hands the dashboard all 28 in one request. Measured on the same page: the roster's read went from 5.2 MB of parse bodies cold to 24 KB. The nine measured rows were identical before and after, on the stale path and on the fresh path.
- **The four-day sign-out fixed at its two causes.** Every sign-out in the Fanout dashboard and extension is now scoped to the one device; the extension's background worker is the only context that rotates the token; a worker with no session makes no pool requests and the panel says "Signed out of the team pool, N items waiting. Sign in again in Settings." The same one-line fix is committed for Radar and OKF, which share the auth project, and waits for Peace's word to deploy.
- **A named competitor counts.** The Prompts and Overview table showed "none" for an answer that named Zendesk, Intercom and Gorgias, because it read what the engine had typed into its searches while the Brands page read the prose. Both now read one predicate. The discovered-brands pass, which had only ever run for simulated runs, now runs on the sweep for captured runs too, bounded per sweep and under the monthly cap.
- **Also live from the first day:** a batch-done email through Resend to the strategist who pressed Run, sent exactly once per batch under concurrency; any team member can invite a client; a loading skeleton for every page so a tab click paints at once; an environment registry naming where every secret lives, pointed to from both project instruction files.
- **Numbers at the end:** 41 commits, 2,264 tests across 136 files, 19 of them against real PostgreSQL; dashboard and runner deployed by Railway snapshot seven times; both live URLs answering 200; seven new patterns in the bug-hunt skill.

### Decisions worth recording

- **Recompute evidence when its inputs change, not when a page opens.** The obvious fix, stamping each parse with its client, was rejected: four writers would have to remember it, one of them a browser extension the owner updates by hand, and 230 existing rows could not be backfilled honestly. The first measurement settled the shape: about 3 captures had ever moved between clients, but 140 of 141 sat under a client edited since, so the stale input is the client's setup and the recompute has to be a background job with a marker the database keeps.
- **No SQL aggregate for the roster.** The numbers come from TypeScript measures shared with every other surface; a second implementation in SQL is the drift this codebase had hit three times. Storing the TypeScript output with a database-computed staleness key keeps one implementation and still reads 28 rows.
- **Review marks are applied at read time from stored keys, never baked into a rollup**, so a Reviewed click updates the count at once without invalidating anything. A seam test proves the stored-key path equals the live path.
- **Trust in SQL comes from the connection role, never from a request setting.** The first guard read `request.jwt.claim.role`, which today's PostgREST no longer sets, so the runner's own writes were judged untrusted and the guard undid them while the log said "healed 100". Caught within minutes by watching the count rather than the log, fixed the same night, and the database test now models today's gateway exactly.
- **Every sign-out on a shared auth project is local to the device.** A global sign-out from Radar or OKF had been revoking the extension's session; the fix is one option on four calls, and a rule in the shared instructions.
- **A signed-out client makes no requests and says so.** Before, 720 anonymous requests a day and a footer reading "Sync pending".

### Frictions and course corrections

- **A projection change passed 2,209 tests and threw on the first render.** Dropping the answer prose from the roster's read looked safe: the four consumer files never touched it. The loader one layer below re-derived every run against its client and counted brand mentions in the prose. Reverted whole, and the rule that a projection is verified in the running app, not in a green suite, is now a pattern.
- **The evidence hash was first a generated column.** The extension pushes client rows back with every column it pulled, and PostgreSQL refuses an explicit value into a generated column, so the next client push would have failed. Caught in the blast-radius sweep before the migration was applied; shipped as a trigger-kept column with a test that a pushed value is overwritten.
- **The brand-pass view offered old parser versions.** Once a capture's newest parse had a pass, the first view offered its older versions next, which would have paid for stale copies of one answer. The PostgreSQL test caught it; a follow-up migration fixed it; the production count of captures waiting fell from 27 to the true 2.
- **The chart took six iterations of Peace's taste** before it was the one red line, one point per day, with a headline number and tooltips, and the table was put back to one row per day after "i didn't ask you to change the table view".
- **"Isn't it instant?"** After Peace signed the extension back in, it drained a four-day backlog one row at a time over her connection, about three minutes, and nothing on screen counted down. Her searches arrived at 16:58 UTC; she had looked before that. The waiting count in the footer is the answer, and it rides her next build.
- **The wrong side of a yes-or-no.** Asked whether reloading the AI tab would fix the "Not connected" pill, the answer was yes; the pill means the capture link to that tab, orphaned by the extension update she had just loaded, and re-injecting was refused by an earlier decision. A wording change to "Reload tab" is proposed.

### Why this matters for the portfolio

- **The council said the plan was wrong, and the count proved it.** Before a line of code, five adversarial readings of the proposed fix converged on one unmeasured number. Measuring it changed the design from a column every writer must remember to a key the database keeps.
- **Correctness never depends on the background job.** A stale row is recomputed at read; a missing rollup is computed live for that client only. The heal and the rollup make the product faster, never truer, so a failed sweep costs seconds, not trust.
- **Two production mistakes of Peace's own assistant are in the record**, each with its detection time, its cause and its test: the trust check that undid its own work, and the view that would have spent money twice. Both were found by watching the database, not the log line that claimed success.
...
---
## 2026-09-16, an archive that rotted from the vendor's side, a jump that landed in a hidden menu, and a proposal built on the client site's own stylesheet

One evening on Notebook Radar's Docs Watch and on stevetoth.ai, opened by Steve's report that the Aug 14 side of an OpenAI policy change rendered with no CSS. The cause was on OpenAI's side, the fix was in how Radar stores a capture, and the day then ran through a second Steve report, a scoping of his ask to put Docs Watch on his public site, an interactive mockup that reuses that site's real CSS, a legal read, and a read-only investigation of the OKF approval queue. Seven Radar commits on the prod branch, 1,518 tests, two Vercel deploys and one crawler snapshot deploy, all verified.

### What shipped

- **Stored captures embed their stylesheets.** Docs Watch had stored each page's HTML and let the archive viewer fetch the vendor's CSS live through a base href. OpenAI's stylesheets are deployment-pinned (`?dpl=`, Vercel skew protection: ignored cross-origin, 404 once the deployment is gone, default lifetime one day). All 14 stylesheets in the Aug 14 capture returned 404 from the crawler; 45 of 56 stored captures carried the same time bomb. `storeVersionHtml` now inlines every stylesheet at capture time (`radar/lib/docwatch/inline-css.ts`, relative `url()` and `@import` made absolute, `</style` escaped, 40-file, 4 MB and 15 s caps, a failed sheet stays a link and is logged). Commit `c37b830a`, crawler redeployed by snapshot so future captures inline at fetch.
- **A backfill rewrote 51 of 52 hotlinked captures in place.** The CSS was fetched through the Railway crawler over SSH because openai.com's Cloudflare wall answers 403 to this laptop's curl and headless Chromium alike, alive or dead. Dead links borrow a donor bundle only when the two documents share at least 90% of their class names: the Aug 14 and Sep 1 versions shared 388 of 391, so the old page wears the newer bundle at 99.2%. One capture is left unstyled (`platform.openai.com/docs/bots`, 2026-07-24); it needs the Internet Archive's asset copies and archive.org was offline all evening.
- **The archive cache is versioned.** The routes send a day-long private cache on purpose (a 2026-08-11 slow-load report). `ARCHIVE_RENDER_REV` now rides every iframe src, link and prefetch, so a rewritten capture is refetched deliberately instead of replaying stale for a day.
- **The jump anchor prefers the change in the content.** Steve's second report: a Gemini docs change opened at the top instead of at the change. The added card title "Streaming" had no matching context, the bare text search hit "Robotics with streaming" inside Google's collapsed left nav, and the first mark in document order takes the anchor, which the browser cannot scroll to. `highlightHtml` now tracks nav, header, footer and aside and prefers matches and the anchor outside them. Verified on the real change (anchor on "Interactions API is now generally available", 12 spans). Commit `19f2848e`.
- **Docs Watch on stevetoth.ai, scoped and mocked.** Steve's ask read as a public `/research` section where a visitor sends a change to their own Claude, not one where visitors author. Repo access confirmed (the site is a monorepo: 16 static pages on nginx rsynced by a GitHub Action, a Next.js twin app, Go and Python services). The proposal (`docs/design/docs-watch-stevetoth-ai/README.md`) is generated static pages from a read-only feed of changes Steve marks published, Send to Claude as static links, the archived before and after reused from Radar. The mockup is one self-contained HTML file on the site's own stylesheet, nav and footer, with eight real changes, the real before/after viewer for the OpenAI ad-policies change, a vendor filter, a behavioral-only switch, word-level side-by-side diffs, version timelines, and one context block behind Chat about this in Claude, Write a post with Claude, and Copy the context (links under 4 KB). Published as an artifact and committed to the repo so it survives an account change.
- **Two diagnoses handed over, not fixed.** A Messenger tip to Steve that the homepage counters index as zeros was confirmed in the raw HTML (six spans ship `0` with the value in a data attribute). The LinkedIn study's missing index was traced to a noindex window: the page went live Sep 10 with a sitewide meta tag, a research-specific tag and an nginx X-Robots-Tag header, all removed by Steve on Sep 13; the action is a Search Console re-request.
- **A read-only OKF investigation, delegated.** A dedicated agent traced today's approvals row by row against Railway deploys and GitHub commits: no approval was lost, but each approval commits to the repo, every commit redeploys prod, and the redeploy kills the commit in flight, so approve-to-live ran up to 2 h 24 m and the panel's polling stopped after five minutes while rows still waited. Report in the OKF repo's handoffs, eight fixes proposed, none applied.

### Decisions worth recording

- **Embed at capture, not at render.** A snapshot that depends on the vendor still serving that build is not a snapshot. Fonts and images stay hotlinked because their loss is cosmetic.
- **Donor bundles are gated on class overlap, never on "same site".** A hashed-CSS-module site cannot be rescued this way and should fall to the archive or the text render rather than wear the wrong CSS quietly.
- **Do not open a pull request in the CTO's repo unprompted.** Steve had tagged Gregory, Gregory wanted to talk to Karla first, and Peace's stated worry was overstepping. The contribution is the diagnosis in the thread plus an offer; the PR waits for a yes.
- **Match the client's site by construction.** The mockup runs on stevetoth.ai's actual stylesheet, snapshotted from the research page, so "does it match" is not a review question. Dribbble supplied four structural references (a month rail beside cards, a filter chip row, a dotted version timeline, a compact entry card) and the glassy dark backgrounds and gradients in those shots were left out on purpose.
- **Send to Claude over a public draft button.** A static prefilled link costs nothing per click and gives an anonymous visitor nothing to abuse; a server-side draft would be spend and surface. The visitor's own Claude gets the before, the after, the summary and both page links.
- **The legal line was drawn before anything public.** Daily fetches of 26 public documentation pages are low risk; republishing full vendor pages publicly is the exposure, along with the crawler's browser user agent. Default to excerpts, attribution and a live link, a named crawler, robots.txt honored, a takedown contact, and a lawyer's read on the full-page viewer. Not lawyer-reviewed; recorded as the working posture.

### Frictions and course corrections

- **The first fix handoff went to the wrong owner.** Peace asked for the diagnosis to be passed to the session already on the bug, then stopped that session and reassigned the work. The message had already been sent; nothing was built twice.
- **Three transport failures before the backfill ran.** The Railway SSH command line breaks on a long JavaScript argument, then caps near 6 KB even base64-encoded, and then the CLI reported "No registered SSH keys found" because a Railway API token in the local env file made it act as a keyless account. Script over stdin, token stripped from the child environment.
- **The build failed on the backfill script**, not the app: Next's type check covers `scripts/` and rejected an environment object without `NODE_ENV`. A cast fixed it.
- **A wrong test, not wrong code.** Two adjacent link texts with no whitespace between them concatenate in the highlighter's shadow text, so a fixture's context never matched and the assertion failed for the right reason. Fixed the fixture.
- **The mockup's tokens vanished on the second build.** The style extractor matched a `<style` inside an HTML comment first, the parser swallowed the `:root` block, and the page fell back to serif with an invisible button. Comments are stripped before extraction now.
- **"Every version of this page" was an in-page anchor**, which Peace read as the before/after button. The real archive viewer was embedded for one change (two renders, 2.3 MB) rather than all eight, to keep the file openable.
- **Chrome was not available all evening** (the browser extension reported disconnected twice), so every site probe, Dribbble search and screenshot ran through headless Playwright.

### Why this matters for the portfolio

- **The root cause was named with the vendor's own documentation, not a guess.** Vercel's skew-protection page states the cross-origin 404 behavior; the crawler's 404s on all 14 links confirmed it. The diagnosis said which probes were meaningless (laptop 403s) and why.
- **A rotting archive was repaired without touching a database row**: same bucket paths, same URLs, a versioned cache key, and a rule for when a neighbor's CSS may stand in for a lost one.
- **Product judgment was exercised on someone else's repo by not committing to it.** The diagnosis was delivered; the fix was offered; the CTO's process was respected.
...
**Addendum, later that night.** The first real code push to the moved repo exposed a gate the read-only checks could not: Railway held both services at "needs approval" because the commit came from Peace's own GitHub account and the single-seat Railway workspace recognises only Steve's. The gate cannot be disabled below Enterprise. Rather than click approve on every push (a forgotten click is a silent non-deploy) or buy a seat, the two Railway-connected repos were pinned with a repository-local credential helper so every push from this machine leaves as Steve's account whatever the shell's default is, and the pin was proven by asking git for credentials with the other account active. Both builds then shipped from the org repo and the live site was probed for the removed code. Two idle Vercel projects were deleted the same night at Peace's call, with the one irreversible consequence, the old app origin going away for good, stated before the click.

## 2026-09-21, a student-facing Docs Watch built and live inside the coaching portal in one evening, after the deploy pipeline it needed was found red

Steve's Docs Watch (a feed of what OpenAI, Google, Anthropic, Bing and Perplexity change in their documentation, with what it means and what to do) had lived in Radar, his internal tool. Peace wanted the coaching students to have their own copy inside the portal they pay for, with no dependency on Radar at runtime, plus two things she added that afternoon: a "Write a post about this" button that hands the change to Claude, and a plain push notification per new change. The plan and a mockup existed from 2026-09-17. The session started from "how do we proceed" and ended with the section live.

### What shipped

- The coaching portal's continuous integration had been red since 2026-09-17 (on pull requests since 2026-09-04) and nothing had deployed since 2026-09-03. Cause: the production-dependency audit gate, tripped by two critical Next.js advisories published 2026-09-08 against the pinned version. A one-line bump to 16.3.5 turned it green and deployed (commit `17bd236`). Peace's hypothesis that the same-day GitHub org move caused it was checked against run dates and ruled out.
- Cohort duplication proven on the live portal, not in tests: Cohort five copied to Cohort six through the real admin form, then compared week for week in the database (8 weeks, 15 content items, 2 videos, 9 files with new storage keys all serving, unlock dates laid from the new start date and correct across the November clock change).
- Docs Watch live at `coaching.notebookers.com/docs-watch` for every role: the feed, the change page with the three Claude actions, a full-page before and after in sandboxed frames, page history, an archive route streaming from R2 with a script-free content security policy. A migration with four tables, a Worker cron every 30 minutes, an admin trigger, opt-in web push with a per-person device cap and a launch floor. First fill: 19 pages, 77 versions, 53 changes, 72 archived page versions. Test suite at 1,823 passing.
- The approved mockup was made clickable first: a working theme toggle, every sidebar row opening a live screenshot of the real page in both themes, the new button on every card, and a push preview.
- An admin launch walk for the new course (a tick-list page and a runbook) covering what only a person can prove: emails, video encoding, a test student end to end.

### Decisions worth recording

- **Build it as six reviewed tasks with a fresh implementer and a fresh reviewer each, then one whole-branch review and one fix wave.** The reviewers found what the implementers had reported as clean: 22 and then 16 forbidden dashes, a missing error boundary, a server-side fetch of an unvalidated push endpoint (a real SSRF), a circular import, a hidden change's headline still rendering on page history, a feed secret that would have travelled with an absolute URL, a string comparison of timestamps in two formats. None reached students.
- **Cap the archive copies at ten per run.** The Worker allows 50 outbound requests per invocation and one copy is three of them; the first live sync copied 19 and then failed every further step. Nothing corrupted, because the design writes the storage key only after the copy succeeds, so a failed step is simply redone.
- **A launch floor in code instead of a data write.** Seven changes were eligible for push at launch, which would have hit the first subscriber as a burst. The obvious fix was a hand update on production rows; the session's write guard refused it. A dated floor in code needs no data write and is the better shape anyway.
- **Externalize the web-push library for the bundler rather than vendor it.** The Cloudflare adapter could not bundle it inline (a NUL-terminated protocol string read as a module path); plain esbuild handles the file, so the fault is upstream. The smallest change that keeps the build honest was chosen and documented, with the condition under which to remove it.
- **Strip dashes from Radar's own prose at the boundary, never from quoted vendor wording.** The house rule applies to the portal's copy; a quotation is not the portal's copy.

### Frictions and course corrections

- The dash gate in every build brief was a `grep -P` command that never runs on macOS (BSD grep); its empty output was read as "clean" by an implementer and, in a different form, by the controller's own diff checks. A reviewer re-ran the exact command and exposed it. The gate now prints a count in Python and the lesson is a named pattern in the bug-hunt skill.
- The stored admin password for headless checks was stale; the portal was driven instead through a one-time sign-in link generated with the service role, disclosed to Peace, with her account shown as the actor in the audit log.
- Two Playwright attempts at the duplication form hit the wrong control (two "Start date" inputs; then the empty form's "Create cohort" button). Neither created anything, verified in the database before the third attempt.

### Why this matters for the portfolio

- **The blocker nobody had recorded was found and fixed before the feature was started.** A red pipeline on a live student product had been invisible for four days; the first hour of the session was spent proving what failed and why, not on the feature.
- **"Does it work?" was answered on the real system.** Duplication was run for real and compared row by row; the feature was walked on the live site at desktop and phone widths, anonymously and signed in, before it was called done.
